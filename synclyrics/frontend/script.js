...
function parseLRC(lrcText) {
    ...
    lines.forEach(line => {
        ...
        if (match) {
            ...
            // Extract Singer Name - Supports @Singer@, [Singer] and Singer:
            const singerMatch = rawText.match(/@(.*?)@/) || rawText.match(/\[(.*?)\]/) || rawText.match(/^(.*?): /);
            if (singerMatch) {
                const foundName = (singerMatch[1] || "").trim();
                // Ensure it looks like a name (not too long) and actually remove the tag
                if (foundName.length > 0 && foundName.length < 20) {
                    lastSinger = foundName;
            ...
            // DIAGNOSTIC: if duoMode is active, prefix the text with the singer name
            if (duoMode && lastSinger) {
                text = `(${lastSinger}) ${text}`;
            }
    ...
    if (currentLyrics.length === 0 && lines.some(l => l.trim().length > 0)) {
        ...
        currentLyrics = lines
            ...
            .map((line, i) => {
                ...
                // Extract Singer Name
                const foundMatch = rawText.match(/@(.*?)@/) || rawText.match(/\[(.*?)\]/) || rawText.match(/^(.*?): /);
                if (foundMatch) {
                    const foundName = (foundMatch[1] || "").trim();
                    if (foundName.length > 0 && foundName.length < 20) {
                        currentSinger = foundName;
                        rawText = rawText.replace(/@.*?@|\[.*?\]|^(.*?): /, '').trim();
                    }
                }
                ...
                if (duoMode && currentSinger) {
                    text = `(${currentSinger}) ${text}`;
                }
