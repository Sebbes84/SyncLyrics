...
def translate_lrc(lrc_text, target_lang="fr"):
    ...
    if not lrc_text or " | " in lrc_text:
        return lrc_text
    ...
    try:
        # Define sample for language detection
        sample = "\n".join(original_texts[:10])
        # Remove singer tags for better detection accuracy
        detect_sample = re.sub(r'@.*?@|\[.*?\]', '', sample)
        
        try:
            from langdetect import detect_langs
            probs = detect_langs(detect_sample)
            logger.info(f"Language detection probabilities: {probs}")
            
            # If the most likely language is target_lang and it's reasonably certain (>90%), skip.
            if probs and probs[0].lang == target_lang and probs[0].prob > 0.90:
                logger.info(f"Lyrics appear to be already in {target_lang}, skipping translation.")
                return lrc_text
        except Exception as e:
...
async def fetch_lyrics(artist: str, title: str, duration: int) -> Optional[str]:
    """Fetch lyrics and systematically translate them if needed."""
    ...
    lyrics = None
    # 1. Check for cached lyrics (consolidated file)
    if os.path.exists(cache_path):
        with open(cache_path, 'r', encoding='utf-8') as f:
            lyrics = f.read()
    
    # 2. If not cached, fetch from internet
    if not lyrics:
        def search():
            try:
        ...
        if lyrics:
            # Initial save of original lyrics
            with open(cache_path, 'w', encoding='utf-8') as f:
                f.write(lyrics)
            cleanup_cache()

    # 3. Systematic Translation: translate if not already translated (Original | Translation format)
    if lyrics and " | " not in lyrics and target_lang:
        logger.info(f"Systematic translation for: {artist} - {title}")
        loop = asyncio.get_event_loop()
        translated_lyrics = await loop.run_in_executor(None, lambda: translate_lrc(lyrics, target_lang))
        
        if translated_lyrics and translated_lyrics != lyrics:
            lyrics = translated_lyrics
            # Update cache file with translated version
            with open(cache_path, 'w', encoding='utf-8') as f:
                f.write(lyrics)
            cleanup_cache()
