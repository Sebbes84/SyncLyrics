...
- **🎤 Mode Duo** : Idéal pour les duos et karaokés.
    - Affichage du nom du chanteur en début de ligne.
    - Héritage automatique : le nom reste affiché jusqu'au prochain changement de chanteur.
    - Supporte plusieurs formats de tags : `@Nom@`, `[Nom]` ou `Nom:`.
- **Ajustement d'Offset** : Possibilité de décaler la synchro manuellement (+/-) pour compenser la latence réseau.
- **Menu Paramètres** : Une interface intégrée pour activer/désactiver les options (Traduction, Mode Duo, Masquage, Jeu) à la volée avec sauvegarde automatique (localStorage).
